...
public class AutenticacionController {
    ...
    public ResponseEntity autenticarUsuario(@RequestBody @Valid 
        ...
        Authentication autenticationToken = new 
            UsernamePasswordAuthenticationToken(datosAutenticacionUsuario.login(), 
            datosAutenticacionUsuario.clave());

        var usuarioAutenticado = authenticationManager.authenticate(autenticationToken);
        var jwtToken = tokenService.generarToken((Usuario) usuarioAutenticado.getPrincipal());
        //para devolver un usuario reconocido en el sistema
